...
export async function uploadPayload(workerUrl: string, userHash: string, payload: SyncifyPayload): Promise<void> {
...
export async function downloadPayload(workerUrl: string, userHash: string): Promise<SyncifyRemoteState> {
  const response = await fetch(workerUrl, {
    method: "GET",
    headers: {
      [USER_HASH_HEADER]: userHash
  ...
  return {
    exists: true,
    payload: (await response.json()) as SyncifyPayload
  };
}
